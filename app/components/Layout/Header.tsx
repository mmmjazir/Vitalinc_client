...
const Header: FC<Props> = ({ activeHeading, navbarOnly }) => {
  ...
  const [socialAuth, { isSuccess, error, isLoading: isSocialAuthLoading }] =
    useSocialAuthMutation();
  ...
  useEffect(() => {
    const runSocialAuth = async () => {
      if (!isLoading && !user && !error && data) {
        setOpen(false);
        await socialAuth({
          email: data?.user?.email,
          name: data?.user?.name,
  ...
  return (
    <div
      className={`${navbarOnly ? "mb-[70px]" : "mb-[136px] max-md:mb-[190px]"}`}
    >
      <div
      ...
      >
        <div
          className={`${styles.section} relative ${styles.normalFlex} justify-between px-4`}
        >
          ...
          <div className={`${styles.normalFlex} hidden md:flex`}>
            <Navbar
              ...
              handleUserLoginCheck={handleUserLoginCheck}
            />
              ...
              <div className="relative cursor-pointer mr-[15px]">
                {isLoading || isFetching ? (
                  ...
                  <div onClick={() => setOpen(true)} className="min-w-[40px]">
                    <IconButton
                      sx={{ color: "white", ":hover": { color: "#269E83" } }}
                    >
                      <CgProfile size={30} />
                    </IconButton>
                  </div>
          ...
          <div className="hidden md:flex md:gap-3 md:items-center">
            <div className="font-Outfit w-[180px] shadow-inner text-[16px] py-2 px-3 rounded-lg">
              {isLoading || isFetching ? (
                ...
                <Link
                  href={`${
                    user?.role === "seller" ? "/seller/shop" : "/shop-create"
                  }`}
                  onClick={(e: any) => {
